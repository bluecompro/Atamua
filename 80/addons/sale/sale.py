...
class sale_order(osv.osv):
    ...
    def _get_total_text(self, cr, uid, ids, field_s, args, context=None):
        ...
        for order in self.browse(cr, uid, ids):
            res[order.id] = num2text.docso(int(math.ceil(order.amount_total)))
        return res

   # _columns = {
    #    'total_text': fields.function(_get_total_text, type='text', string=u'T?ng ti?n b?ng ch?', method=True),
	# }
...
#<t>cmt Sy tao mail hoi gia
    def wkf_send_rfq(self, cr, uid, ids, context=None):
        ...
        try:
            ...
            template_id = False
        try:
            compose_form_id = ir_model_data.get_object_reference(cr, uid, 'mail', 'email_compose_message_wizard_form_RFQ')[1]
        except ValueError:
